...
# function to get all the playable moves (will return list of the slots that can be played in)
def getvalidpositions():
    positions = []
    slotnum = 0
    for slot in board[0]:
        if slot == 0:
            positions.append(slotnum)
        slotnum = slotnum + 1
    return positions

# function to place the piece in a slot
def place(move, turn):
    rown = 0
    for row in board:
        if rown == height - 1:
...
while True:
    ...
    if turn == 2:
        ...
        playablepositions = getvalidpositions()
        # generate a random number (between 0 and how many playable moves there are)
        move = random.randint(0, len(playablepositions) - 1)
        # place the piece with the random move
        place(playablepositions[move], turn)
